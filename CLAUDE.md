...
The driving input function also forces values at the end (a vtable check that zeroes pedals and applies the handbrake, and an accelerate-quickhack path that forces full throttle). The scripts treat quickhacks, remote control and the `NoDriving`/`VehicleOnlyForward` player status effect tags as "driving not allowed", and AutoDrive too through `AutoDriveSystem.GetAutodriveEnabled()` (`VehicleObject.IsAutoDriveModeEnabled`, which the plugin checks every tick, stays false during the patch 2.3 AutoDrive, confirmed in-game). AutoDrive drives the car natively and ignores the input block, and a pedal at 10% or more switches it off. This keeps shaping and cruise control stay out of those states. The speed limiter waits through them instead of switching off, since it only lowers the throttle.
